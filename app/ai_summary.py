...
1а. Проценты не считай сам — бери готовые поля kpi_pct, budget_pct, unit_cost_pct,
   delivery_pct и умножай на 100. Пересчёт в уме даёт ошибки (был случай: по данным
   +11.3%, в тексте оказалось +12.5%), а читатель сверяет текст с таблицей.
1в. Названия полей JSON в тексте не упоминай: никаких «kpi_pct 0.055»,
   «delivery_pct», «by_placement_closed». Отчёт читает клиент, а не разработчик —
   пиши «KPI перевыполнен на 5.5%», «по сводке за закрытые недели».
1б. Отчёт показывает только месяц: недельных таблиц в нём нет. Недельные числа из
   поля weeks использовать можно и нужно, чтобы объяснить причину, но говори прямо,
   что это разбивка из исходного файла («по неделям в файле…»), иначе читатель будет
   искать эти цифры в отчёте и не найдёт.
...
5. Не пиши наблюдений про надёжность источника, методику и перепроверку данных —
   выводы только про кампанию и спрос. Оговорка про источник уже есть в самом блоке
   спроса, дублировать её отдельным наблюдением не нужно.
5а. Блок search_demand.groups.brands — наш бренд и конкуренты: доля голоса в поиске
   (our_share), место (our_rank) и объёмы. Если groups.brands.missing > 0, упомяни,
   что часть ключей без данных. Это спрос в Google, а не доля рынка — так и называй.
   Если our_rank_tied_with непустой, объёмы равны: пиши «делит место с ...», не выдумывай
   разницу и не объясняй её. Keyword Planner округляет объёмы корзинами, поэтому равные
   числа у разных брендов — норма.
